...
For these purposes, literature and industry increasingly adopt \acfp{CRDT} to design systems.
\acp{CRDT} are new specifications of existing data types, e.g. the Set or the Sequence.
While \acp{CRDT} have the same behaviour as previous specifications in sequential executions, they actually shine in distributed settings as they natively support concurrent modifications.
...
\IEEEPARstart{W}{hen} creating distributed systems, designers have to make a tradeoff between consistency and latency \cite{pacelc2012}.
Many systems choose to favour latency and thus adopt optimistic replication techniques \cite{10.1145/1057977.1057980}.
This approach ensures the high availability of the system, even in case of network partitions.
To this end, it relaxes consistency constraints and minimises coordination between nodes.
In this model, every node owns a copy of the data, can modify it and then propagate updates to others.
...
From users' perspective, \acp{CRDT} share the same semantics and interfaces as previously existing specifications.
However, their pecularity is that \acp{CRDT} are designed to natively support concurrent modifications.
\acp{CRDT} thus have the same behavior as previous specifications in sequential executions, but also define additional semantics for scenarios that can only occur in distributed executions.

To this end, \acp{CRDT} embed a conflict resolution mechanism directly in their specification.
...
This property makes \acp{CRDT} particularly suited to the design of highly-available large-scale distributed systems.
...
It requires the definition of correct and efficient conflict resolution mechanisms that preserve users' intention.
Also, designed systems must guarantee privacy and be resilient to censorship.
Work \cite{ahmednacer:inria-00629503,lseq2017} has demonstrated the relevance of Sequence \acp{CRDT} to address these issues, notably thanks to their compatibility with peer-to-peer approaches.
As a result, several collaborative editors using \acp{CRDT} were proposed \cite{Nedelec2016CRATE, MUTE2017}.
...
To address this issue, we propose a new Sequence \ac{CRDT}: RenamableLogootSplit (RLS).
This novel data structure embeds a renaming mechanism into its specification.
The renaming mechanism allows the data structure to punctually reduce its metadata and to minimise it eventually.
While this kind of approach has been proposed in previous work \cite{letia:hal-01248270,zawirski:hal-01248197}, RenamableLogootSplit differs as its renaming mechanism does not require synchronous coordination.
...
Section \ref{sec:evaluation} presents the experimental evaluation of RenamableLogootSplit to which we proceeded and the obtained results.
Section \ref{sec:discussion} discusses several tradeoffs that RenamableLogootSplit implementations can make.
Section \ref{sec:related-work} compares our approach to related work.
Finally Section \ref{sec:conclusion} summarises our work and introduces possible future work.
...
In order to measure the overhead introduced, we proceeded to benchmarks.
We present obtained results in \autoref{fig:evolution-document-size}.
We measured that content, represented as the blue dashed line, eventually represents less than 1\% of the whole data structure size, displayed as the red line.
...
\mnnote{QUESTION : Développer ce paragraphe en une sous-section de \autoref{sec:discussion} ? Je pourrais préciser les différents aspects du mécanisme de compression et de décompression (éléments permettant d'identifier un bloc de manière unique, algorithme de décompression, modèle de cohérence requis...)}
...
\mnnote{QUESTION : Ajouter une sous-section sur la stratégie de renommage ? À ce stade, me permettrait d'introduire le requirement de performance (temps d'intégration, mais aussi consommation en bande-passante ou même taille de la structure en mémoire). Permettrait aussi de faire le lien avec \autoref{sec:evaluation}.}
...
We define the one \emph{rename} operation that nodes apply from a set of concurrent \emph{rename} operations as the \emph{primary} one.
...
Upon the reception of \emph{rename} operations, nodes can only rely on their current knowledge to make decisions.
However, nodes have only partial knowledge of the state of the system and of the operations issued at a given time.
They may thus consider a received \emph{rename} operation as the \emph{primary} one and apply it, only to learn a few moments later that there is a concurrent \emph{rename} operation with higher priority.
In that case, nodes have to revert mistakenly applied \emph{rename} operations to apply the correct \emph{primary} one eventually.
...
As stated in \autoref{sec:dealing-with-concurrent-updates}, nodes build and maintain the \emph{epoch chain} from received \emph{rename} operations.
This data structure orders epochs according to their \emph{parent-child} relation.
...
\begin{myalgorithm}[h!]
    \begin{algorithmic}
        ...
        \EndFunction
    \end{algorithmic}
    \caption{Updated algorithms to rename an identifier}

...
To enable every nodes to select in a coordination-free manner the same target epoch, we define \emph{priority}, a strict total order relation on epochs.
%
% \begin{definition}[Priority relation]
%     A strict total order $<$ on the set of epochs $E$.
%     It enables nodes to compare any pair of epochs $e$ and $e'$.
%     \[
%         \forall e, e' \in E \cdot e \neq e' \implies e < e' \lor e' < e
%     \]
% \end{definition}
%
...
%     The epoch $e_{target}$ from the set of epochs $E$ towards which nodes should progress.
%     \[
%         e = e_{target} \implies e \in E \land \nexists e' \in E \cdot e < e'
%     \]
% \end{definition}
...
For other identifiers such as \id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}, \textsc{revertRenameId} removes the prefix (\id{i}{A1}{1}) to isolate the tail (\id{i}{B0}{0}\id{m}{B1}{0}).
...
%     \[
%         E' = \{ e \in E | e = e_{max\_cs} \lor e_{max\_cs} < e \}
%     \]
% \end{definition}
...
\mnnote{QUESTION:
    Dans le paragraphe d'après, j'explique que les noeuds ne renomment en fait pas leur état dans tous les cas.
    Transformer le "have to browse and rename..." en "may have to browse..." plutôt ?
    Ou ça serait plus confusant qu'autre chose ?
}
...
Since these operations introduce epochs that are not selected as the new target epoch, nodes does not actually proceed to rename their state.
The application of \emph{concurrent with lower priority rename} operations hence solely consists of adding the introduced epoch and the corresponding \emph{former state} into the epoch tree.

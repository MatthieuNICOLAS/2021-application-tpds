...
\begin{figure*}[t!]
    ...
    \begin{tikzpicture}
        ...
        \path
            ...
            to ++ (0:8 * \widthletter) node[epoch] (S1B-left) {\epoch{B2}}
            to ++ (0:1.3 * \widthepoch) node[block, fill=mydarkpurple,
                    label={ [] below:{\color{mydarkpurpleid}\id{i}{B2}{0..4}} }
                ]  {HELLO};

        \draw[dotted] (S0A-right) -- node[above, align=center] {\emph{rename to \epoch{B2}}} (S1A);
        \draw[dotted] (S0B-right) -- node[below, align=center]{\emph{rename to \epoch{A1}}} (S1B-left);
        ...
        \draw[dashed, ->, thick, shorten >= 3] (S0A-right.east) -- (S1B-left.west);
...
\mnnote{TODO: Expliquer qu'en fonction de la relation \emph{priority} choisie, peut être amenée à rejouer un renommage qu'on avait annulé précédemment. Dans ce cas, pour assurer la convergence, le couple (renameId, revertRenameId) doivent être des fonctions inverses. On doit donc proposer une nouvelle version de renameId pour gérer correctement les identifiants insérés après le renommage initial. Cette nouvelle version se trouve en annexe.}

...
This figure depicts the same scenario as in \Cref{fig:concurrent-insert-rename-inconsistent}, except that this time node A uses \textsc{renameId} to rename the concurrently generated id before inserting it in its state.
...
For other identifiers such as \id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}, \textsc{revertRenameId} removes its prefix (\id{i}{A1}{1}) to isolate its tail (\id{i}{B0}{0}\id{m}{B1}{0}).
...
We denote as \emph{required epochs} the set of epochs that form the paths between every \emph{potential current epochs} and their \ac{LCA}.
...
Node A then proceed to compute \emph{required epochs}.
...
From its knowledge, node B computes that \epoch{A1}, \epoch{B2}, \epoch{B7} and \epoch{B9} form the \emph{potentiel current epochs}.
...
\mnnote{QUESTION:
    Ajouter une sous-section sur une version améliorée de \emph{renameId()} permettant de renommer par bloc plutôt que par id ?
...
\begin{enumerate*}
    \item \emph{simple remote} denotes the remote \emph{rename} operations that introduce children epochs of the node's current one
    \item \emph{concurrent with higher priority} the remote \emph{rename} operations that introduce sibling epochs with higher priorities than the node's current one
    \item \emph{concurrent with lower priority} the remote \emph{rename} operations that introduce sibling epochs with lower priorities than the node's current one.
\end{enumerate*}
...
\begin{table}[!ht]
    ...
    \resizebox{\columnwidth}{!}{
        \begin{tabular}{lrrrrr}
            ...
            \cmidrule(lr){1-6}
            Simple remote & 30  &   481.32 &   477.13 &     537.30 &  17.11 \\
            ...
            \cmidrule(lr){1-6}
            Cc. with higher priority  & 30  &   643.53 &   643.57 &     682.80 &  13.42 \\
            ...
            \cmidrule(lr){1-6}
            Cc. with lower priority  & 30  &     1.36 &     1.30 &       3.53 &   0.37 \\
...
Local \emph{rename} operations take hundreds of milliseconds while \emph{simple remote} ones and \emph{concurrent with higher priority} ones may last seconds if delayed for too long.
...
Another interesting result from this benchmark is that \emph{concurrent with lower priority rename} operations are cheap to apply, since their application actually consists of storing their embedded \emph{former states}.
...
To enable greater flexibility in the design of the \emph{priority} relation, it is necessary to remove the constraint that prohibits nodes from redoing \emph{rename} operations.
...
As shown in \Cref{tab:evolution-integration-time-rename}, integrating \emph{concurrent with higher priority rename} operations is expensive as nodes have to browse and rename their whole current state.
This process can introduce a signification computational overhead in some cases.
Especially, a node may receive concurrent \emph{rename} operations in the reverse order to the one defined by the \emph{priority} relation over their introduced epochs .
...
On the other hand \emph{concurrent with lower priority rename} operations are cheap to integrate, as nodes simply add to their state a reference to the corresponding \emph{former state}.
...
First, nodes process \emph{rename} operations as \emph{concurrent with lower priority} ones.
...
Then each node keeps track of a level of confidence in the current target epoch, computed for example from the time elapsed since the node received a concurrent \emph{rename} operation and the number of online nodes still known as using the \emph{parent} epoch.
Once the level of confidence reaches a given threshold, the node renames its state to the target epoch.
...
In return, the strategy reduces the probability of erroneously integrating \emph{rename} operations as \emph{concurrent with higher priority} ones.

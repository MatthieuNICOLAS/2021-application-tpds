        ...
        \begin{minipage}{\linewidth}
            ...
            \begin{tikzpicture}[scale=0.8,every node/.style={scale=0.8}]
                \path
                    ...
                    to ++(0:3) node[point, label=above:{rename to \epoch{A9}}] (a9) {}
                    to ++(0:1.5) node (a-end) {};
                ...
                \path
                    ...
                    to ++(0:3) node[point, label=below:{rename to \epoch{B8}}] (b8) {}
                    to ++(0:1.5) node (b-end) {};
...
\emph{Concurrent with higher} (resp. \emph{lower}) \emph{priority} designates a remote \emph{rename} operation that introduces a new epoch sibling of the current node epoch with higher (resp. lower) priority.
...
Local \emph{rename} operations take hundreds of milliseconds while \emph{direct remote} ones and \emph{concurrent with higher priority} ones may last seconds if delayed for too long.
...
\mnnote{TODO:
    Retravailler section avec plan suivant.
    Indiquer que passe son temps à sauter d'une epoch à l'autre.
    On va postpone le renommage de l'état vers l'epoch target en attendant d'avoir une haute confiance envers l'epoch.
    Mais pour ne pas être bloqué, on s'autorise de rename les opérations vers mon epoch courante.
    Ajouter argumentaire pour montrer que le coût de transformation des opérations est négligeable par rapport au coût d'un mauvais rename.
}

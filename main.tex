...
While the ever-growing metadata decreases the memory efficiency of these data structures, it also increases their bandwidth consumption and computational overhead.
...
It enables to significantly reduce the memory overhead of the data structure.

\subsection{Limits}

As previously stated, the size of identifiers from a dense total order is variable.
...
Proceeding this way also enables us to assign an unique triplet $\langle \trm{node~id}, \trm{~seq}, \trm{offset} \rangle$ to every identifier, thus ensuring their uniqueness as required by \Cref{prop:uniqueness}.
...
As described by this example, naively applying concurrent updates would result in inconsistencies and conflict with \Cref{prop:intention}.
...
Initially, the replicated sequence starts at the \emph{origin} epoch noted \epoch{0}.
...
Additionally, we present a mechanism to garbage collect them in \Cref{sec:gc-former-states}.
...
At the end of the scenario described in \Cref{fig:conflicting-rename-operations}, nodes A and B are at epochs \epoch{A1} and \epoch{B2} respectively.
...
Since $\trm{newId}$ has been inserted after the \emph{rename} operation, there is no existing contraint on the returned identifier except the \Cref{prop:order}.
...
Instead, we compare them in the form of a discussion in \Cref{sec:core-nebula}.
...
% To this end, we applied randomly generated operations on snapshots taken at different points of the simulations.
...
\begin{table}[!ht]
    ...
    \resizebox{\columnwidth}{!}{
        \begin{tabular}{lrrrrrr}
            \toprule
            \multicolumn{2}{c}{Parameters} & \multicolumn{5}{c}{Integration Time (ms)} \\
            \cmidrule(lr){1-2} \cmidrule(lr){3-7}
            Type & Nb Ops (k) &  Mean & Median &    IQR & 1\textsuperscript{st} Quant. & 99\textsuperscript{th} Quant. \\
            \midrule
            ...
            \cmidrule{1-7}
            Cc. int. lesser epoch & 30  & 1.36 &    1.3 & 0.038 &       1.22 &        3.53 \\
...
Obtained results are shown in \Cref{fig:time-to-replay-log}.
...
Another solution, which we present in \Cref{sec:compression-rename-op}, is to implement a compression mechanism to send only the needed components to identify each block of the \emph{former state}.
...
Regarding performance, this solution behaves as RenamableLogootSplit with one \emph{renaming bot} (cf. \Cref{sec:evaluation-results}), but with an additional overhead corresponding to the cost of the chosen consensus protocol.
...
\newcommand{\textmrm}[1]{$\mathrm{#1}$}

\noindent \textmrm{position} : Retrieves the value of the position of the first tuple of the given id.
    e.g. \textmrm{position}(\id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}) returns i.

\noindent \textmrm{findIndex} : Retrieves the index of the given id in the given list of ids renamedIds.

\noindent \textmrm{findIndexOfPred} : Retrieves the index of the predecessor of the given id in the given list of ids renamedIds.

\noindent \textmrm{concat} : Returns a new id made of the tuples of given ids.
    e.g. \textmrm{concat}(\id{i}{A1}{1}, \id{i}{B0}{0}\id{m}{B1}{0}) returns \id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}.

\noindent \textmrm{isRenamedId} : Indicates if the given id is part of the block resulting of the \emph{rename} operation, \ie id is of the form \id{pos}{nId~nSeq}{offset} with \emph{pos} corresponding to the position of the first renamed id, \emph{nId} to the node id of the author of the \emph{rename} operation and \emph{nSeq} to its sequence number at the time.

\noindent \textmrm{getFirstOffset} : Retrieves the value of the offset of the first tuple of the given id.
    e.g. \textmrm{getFirstOffset}(\id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}) returns 1.

\noindent \textmrm{getTail} : Returns a new id obtained by removing the n first tuples of the given id.
    e.g. \textmrm{getTail}(\id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}, 1) returns \id{i}{B0}{0}\id{m}{B1}{0}.

\noindent \textmrm{getLastOffset} : Retrieves the value of the offset of the last tuple of the given id.
    e.g. \textmrm{getLastOffset}(\id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}) returns 0.

\noindent \textmrm{createIdFromBase} : Returns a new id by copying the given id and replacing its last offset with the given one.
    e.g. \textmrm{createIdFromBase}(\id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}, 5) returns \id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{5}.

\noindent \textmrm{isPrefix} : Indicates if the given id is a prefix of the other given id.
    e.g. \textmrm{isPrefix}(\id{i}{A1}{1}\id{i}{B0}{0}, \id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}) returns true.
...
\Cref{alg:appendix-rename-id} illustrates the remaining functions composing \textsc{renameId}.
    ...
    \begin{algorithmic}
        \Function{revRenIdLessThanNewFirstId}{id, firstId, newFirstId}
            ...
            \If{isPrefix(predNewFirstId, id)}
                ...
                \If{tail < firstId}
                    \State \Return tail
                \Else \Comment{$id$ was inserted causally after the \emph{rename} op}
%                    \State \Comment{$id$ has been inserted causally after the \emph{rename} op}
        ...
        \Function{revRenIdGreaterThanNewLastId}{id, lastId}
            \If{id < lastId} \Comment{$id$ was inserted causally after the \emph{rename} op}
%                \State \Comment{$id$ has been inserted causally after the \emph{rename} op}
                \State \Return concat(lastId, MIN\_TUPLE, id)
            \ElsIf{isPrefix(newLastId, id)}
                \State tail $\gets$ getTail(id, 1)
                \If{tail < lastId} \Comment{$id$ was inserted causally after the \emph{rename} op}
%                    \State \Comment{$id$ has been inserted causally after the \emph{rename} op}

...
\Cref{sec:discussion} discusses several tradeoffs that RenamableLogootSplit implementations offer.
...
To this end, it applies \textsc{revertRenameId} to every identifier of its current state.

\textsc{revertRenameId} determines which strategy to apply to revert the given identifier using patterns.
Before going into detail, we add the following notation : we name $\trm{newX}$ identifiers in the resulting epoch of the \emph{rename} operation, while $\trm{X}$ refer to their counterpart in the preceding epoch.

An identifier $\trm{newId_1}$ of the form \id{pos}{nId~nSeq}{offset} (\id{i}{A1}{offset} in the current example) corresponds to the new value of an identifier directly renamed by the \emph{rename} operation.
To retrieve $\trm{id_1}$, \textsc{revertRenameId} simply uses the offset of $\trm{newId_1}$ since it corresponds to the index of $\trm{id_1}$ in the \emph{former state} (\ie renamedIds[offset] = id).

Case of identifiers $\trm{newId_Y}$ of the form \id{pos}{nId~nSeq}{offset}$\trm{tail}$ (\eg \id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}) is divided into several sub-cases.
In every of these subcases, we have \[\trm{newId_Y} \in ]\trm{newPredId}, \trm{newSuccId}[\]
As thus, according to \Cref{prop:order}, we must have \[\trm{id_Y} \in ]\trm{predId}, \trm{succId}[\]

The first subcase is identifiers $\trm{newId_2} \in ]\trm{newPredId~predId}, \trm{newPredId~succId}[$.
For these identifiers, we note that :
\begin{enumerate*}[label=(\roman*)]
    \item we have $\trm{tail} \in ]\trm{predId}, \trm{succId}[$
    \item $\trm{newId_2}$ may be the result of an \emph{insert} operation concurrent to the \emph{rename} one.
        In such case, $\trm{newId_2}$ has been obtained using \textsc{renIdFromPredId} and we have $\trm{id_2} = \trm{tail}$.
\end{enumerate*}
...
The second subcase is identifiers $\trm{newId_3} \in ]\trm{newPredId}, \trm{newPredId~predId}[$.
In order to comply with \Cref{prop:order}, we must have $\trm{id_3} \in ]\trm{predId}, \trm{id_2}[$.
For these identifiers, we note that :
\begin{enumerate*}[label=(\roman*)]
    \item $\trm{newId_3}$ is of the form $\trm{newPredId~tail}$ with $\trm{tail} < \trm{predId}$.
        We cannot thus simply return $\trm{tail}$ in this case.
    \item $\trm{newId_3}$ may have only been inserted causally after the \emph{rename} operation.
        Therefore, there is no existing contraint on the returned identifier except the \Cref{prop:order}.
\end{enumerate*}
...
The last subcase is identifiers $\trm{newId_4} \in ]\trm{newPredId~succId}, \trm{newSuccId}[$.
Here, we must have $\trm{id_4} \in ]\trm{id_2}, \trm{succId}[$.
Note that this subcase is the counterpart of the previous one.
The logic to handle it is thus the same : adding a prefix enforcing the intended order.

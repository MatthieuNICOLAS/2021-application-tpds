...
\begin{figure}[t!]
    ...
    \subfloat[State of A's \emph{epoch tree} at (1)]{
        \begin{tikzpicture}
            \path
                node[op] (e0) {\epoch{0}}
                to ++(270:1.5) node[op] (eA1) {\epoch{A1}};
    ...
    \subfloat[State of B's \emph{epoch tree} at (1)]{
        \begin{tikzpicture}
            \path
                node[op] (e0) {\epoch{0}}
                to ++(270:1.5) node[op] (eB2) {\epoch{B2}};
...
\end{figure}
\mnnote{TODO: Faire apparaître la relation \emph{priority} entre les epochs ainsi que l'epoch courante dans \autoref{fig:priority-example}}
...
Node A's (resp. node B's) \emph{epoch tree} is thus only composed of the epochs \epoch{0} and \epoch{A1} (resp. \epoch{B2}).
...
We explain how node A does it in \autoref{sec:applying-primary-rename-op}.
...
Upon the reception of a \emph{rename} operation introducing the new \emph{primary epoch}, nodes have then to determine if they applied concurrent and conflicting \emph{rename} operations previously.
...
In order to switch to the new \emph{primary} epoch, nodes first revert the effects of concurrent \emph{rename} operations.
...
As explained in \cite{nicolas:hal-02526724} and \autoref{sec:applying-primary-rename-op}, nodes have to store epochs and corresponding \emph{former states} to transform operations from previous or concurrent epochs to the current one, or to rename their state to switch to a new \emph{primary} epoch.

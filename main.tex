...
An example of its usage is illustrated in \Cref{fig:concurrent-insert-rename-fixed}.
This figure depicts the same scenario as in \Cref{fig:concurrent-insert-rename-inconsistent}, except that this time node A uses \Cref{alg:renameId} to rename the concurrently generated id before inserting it in its state.
...
\begin{figure*}[!ht]
    ...
    \end{tikzpicture}
    \caption{Renaming concurrent update using \Cref{alg:renameId} before applying it to maintain intended order}
    \label{fig:concurrent-insert-rename-fixed}
\end{figure*}

\Cref{alg:renameId} also enables nodes to handle the opposite case : to integrate remote \emph{rename} operations on their local state while they have previously applied concurrent updates.
This case corresponds to node B's one in \Cref{fig:concurrent-insert-rename-fixed}.
Upon the delivery of node A's \emph{rename} operation, applying \Cref{alg:renameId} to every identifiers of its state would enable node B to reach an equivalent state to node A's one.
...
\Cref{fig:conflicting-rename-operations} expands the scenario previously described in \Cref{fig:concurrent-insert-rename-fixed} to depicts such one.
...
Nodes then select the maximum epoch according to this relation as the target epoch.
...
\Cref{fig:priority-example} provides an example of its use.
        ...
        \begin{minipage}{\linewidth}
            ...
            \begin{tikzpicture}[scale=0.8,every node/.style={scale=0.8}]
                ...
                \path
                    ...
                    to ++(0:1) node (b0) {}
                    to ++(0:1) node[point, label=above:{rename to \epoch{B2}}] (b2) {}
                ...
                \path
                    ...
                    to ++(0:1) node (c0) {}
                    to ++(0:4) node[point, label=below:{rename to \epoch{C6}}] (c6) {}
                    to ++(0:2) node (c-end) {};
...
This topic will be further discussed in \Cref{sec:designing-more-effective-priority-relation}.
...
Described in \Cref{alg:revertRenameId}, this function enables nodes to revert a previously applied \emph{rename} operation.
...
\end{enumerate*}
We illustrate its behaviour with the help of \Cref{fig:revertRenameId}.
...
This figure resumes the scenario from \Cref{fig:conflicting-rename-operations}.
Node A receives node B's \emph{rename} operation, which is concurrent to the \emph{rename} operation node A previously applied.
According to the proposed \emph{priority} relation, node A determines the introduced epoch \epoch{B2} as the target one.
It thus proceeds to revert its state to an equivalent one at \epoch{0}, the \ac{LCA} of its current epoch \epoch{A1} and the target epoch \epoch{B2}.
To this end, it applies \textsc{revertRenameId} to every identifiers of its current state.

\textsc{revertRenameId} determines which strategy to apply to revert the given identifier using patterns.
For example, identifiers of the form \id{pos}{nId~nSeq}{offset} (\id{i}{A1}{offset} in the current example) corresponds to the new values of the identifiers which compose the \emph{former state}.
To retrieve the original identifiers, \textsc{revertRenameId} simply uses their offset since it is their index in the \emph{former state}.

For other identifiers such as \id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}, \Cref{alg:revertRenameId} removes its prefix (\id{i}{A1}{1}) to isolate its tail (\id{i}{B0}{0}\id{m}{B1}{0}).
...
\mnnote{TODO: Indiquer que retirer le préfixe permet d'inverser la transformation appliquée à l'identifiant par $\trm{renIdFromPredId}$ dans le cas où celui-ci a été inséré en concurrence du renommage}
...
To this end, \Cref{alg:revertRenameId} generates the resulting identifier from the identifier of the predecessor or by derivating the successor one, and exclusive tuples of the renaming mechanism, $\trm{MIN\_TUPLE}$ and $\trm{MAX\_TUPLE}$.

Once node A mapped its state to the equivalent one at \epoch{0} using \Cref{alg:revertRenameId}, it can now apply \textsc{renameId} to compute the corresponding state at \epoch{B2}.
...
Functions to handle the rest of cases are featured in \Cref{app:revert-rename-id}.
...
But on the other hand, \textsc{renameId} does not do the same for identifiers inserted causally after the \emph{rename} operation.
...
It would be interesting to remove this constraint to propose more effective \emph{priority} relations, as discussed in \Cref{sec:designing-more-effective-priority-relation}.
It can be achieved by designing a set of inverse functions for \textsc{renameId} and \textsc{revertRenameId}.
...
\mnnote{TODO: Déplacer ce paragraphe (En 7.3 ? Une sous-section de la 7 dédiée ?)}
...
% - Indiquer qu'on veut GC les métadonnées du mécanismes de renommage
% - Alors que le système progresse, des epochs et leurs données associées deviennent inutiles
% - Doit identifier de quelles epochs on peut se séparer
...
% - Pour cela, repose sur la notion de stabilité causale des opérations
% - Une fois qu'une opération /rename/ est stable, cela signifie que tous les noeuds ont au moins progressé sur l'epoch correspondante
%   - Ou une epoch plus grande d'après la relation /priority/
% - Permet donc d'identifier l'ensemble des /potential current epochs/
...
An operation being causally stable means it was delivered to every nodes.
...
Using this knowledge, nodes determine the \emph{potential current epochs}, the set of epochs on which nodes may currently be.

% - Besoin de maintenir un chemin entre ces epochs
% - Identifie donc leur LCA
% - Définit les /required epochs/ comme l'ensemble des epochs qui appartiennent aux chemins entre LCA et les /potential current epochs/
% - Peut GC toute epoch n'appartenant pas à /required epochs/
To handle next operations, nodes have to maintain the paths between every epoch of \emph{potential current epochs}.
We denote as \emph{required epochs} the set of epochs that form the paths between \emph{potential current epochs} and their \ac{LCA}.
...
% - Faire remarquer que dans le cas sans operations /rename/ concurrentes, le LCA des /potential current epochs/ correspond à la /maximum causally stable epoch/
%   - Cette epoch devient donc la nouvelle racine de l'/epoch tree/
\begin{figure}[!ht]
    \subfloat[Execution]{
        \begin{minipage}{\linewidth}
            \centering
            \begin{tikzpicture}[scale=0.8,every node/.style={scale=0.8}]
                ...
                \path
                    ...
                    to ++(90:1)
                    to ++(0:1.5) node[point, label=below:{rename to \epoch{B8}}] (b8) {}
                    to ++(0:1.5) node (b-end) {};

                \draw[->, thick] (b0) -- (b2) -- (b3);
                \draw[dotted] (b3) -- (b6);
                \draw[->, thick] (b6) -- (b7) -- (b8) -- (b-end);
    ...
    \subfloat[States of respective epoch trees with \emph{potential current epochs} and \emph{required epochs} displayed]{
        \begin{minipage}{\linewidth}
            ...
            \begin{tikzpicture}[scale=0.8,every node/.style={scale=0.8}]
                ...
                \draw[thick, ->] (Be0) edge (BeB2) (BeB2) edge (BeB7) (BeB7) edge (BeB8) (Be0) edge (BeA1);
...
In \Cref{fig:GC-execution}, we represent an execution in which two nodes A and B respectively issue two \emph{rename} operation.
Then they each receive each other's first \emph{rename} operation before issuing their own third \emph{rename} operation.
In \Cref{fig:GC-epoch-trees}, we represent the states of their respective \emph{epoch trees}.
...
Epochs \epoch{B2} and \epoch{A9} form the set of \emph{potential epochs} and nodes can now only issue operations from these epochs or one of their not yet known descendants.
% As nodes need to maintain the path between \epoch{D10} and others, ...
\mnnote{TODO: Expliquer que \emph{required epochs} correspond à \emph{potential current epochs} dans ce cas}
Node A infers that epochs \epoch{0}, \epoch{A1} and \epoch{A8} can be safely garbage collected.

\mnnote{TODO: Présenter le cas du point de vue de B}
...
\mnnote{QUESTION:
    Exemple vraiment pertinent ?
    Est-ce qu'un seul epoch tree plus complet avec \emph{potential current epochs} différent de \emph{required epochs} et des epochs à GC ne seraient pas plus intéressant ?
    Est-ce utile d'adopter le point de vue de différents noeuds ?
}
...
Nodes can then to garbage collect all other epochs and associated metadata, effectively suppressing the overhead of the renaming mechanism.
...
\mnnote{QUESTION: Réintroduire le fait que la stabilité causale nécessite un protocole de membership ?}
...
\mnnote{TODO: Refaire figure pour déplacer légende en-dessous}
...
Since the operation has to be transformed beforehand using \Cref{alg:renameId}, we observe a computational overhead compared to other operations.
...
Regarding operations from concurrent epochs, we observe an additional overhead as nodes have first to reverse the effect of the concurrent \emph{rename} operation using \Cref{alg:revertRenameId}.
...
We will discuss further this topic in \Cref{sec:postponing-transition-to-new-epoch}.
...
\Cref{fig:evolution-document-size} shows that concurrent \emph{rename} operations decrease RenamableLogootSplit's performances.
...
Although the \emph{priority} relation proposed in \Cref{sec:priority} is simple and ensures that nodes designate the same epoch as the \emph{primary} one, it introduces a significant computational overhead in some cases.
...
As shown in \Cref{tab:evolution-integration-time-rename}, integrating \emph{primary rename} operations is expensive as nodes have to browse and rename their whole current state.
...
The use of a consensus protocol to regulate \emph{rename} operations enables systems to discard all parts dedicated to the handling of concurrent \emph{rename} operations, \ie the design of a \emph{priority} relation and the implementation of \Cref{alg:revertRenameId} and \Cref{rule:gc-concurrent-primary-epoch}.

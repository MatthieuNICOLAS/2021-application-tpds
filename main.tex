...
% Before:
% As explained in \Cref{sec:rename-op}, some nodes may have applied concurrent \emph{insert} operations to their state before receiving a given remote \emph{rename} operation.
% \Cref{alg:renameId} also solves this case.
...
% To revert the effect of the \emph{rename} operation, \textsc{revertRenameId} returns the tail.
% % To revert the effect of the \emph{rename} operation, \textsc{revertRenameId} first removes its prefix (\id{i}{A1}{1}) to isolate its tail (\id{i}{B0}{0}\id{m}{B1}{0}).
...
% \textsc{revertRenameId} can thus return the tail, as it
...
\Cref{fig:GC-epochs} illustrates a use case of \Cref{rule:gc-concurrent-primary-epoch} and \Cref{rule:gc-root}.
In \Cref{fig:GC-execution}, we represent an execution in which two nodes A and B respectively issue two \emph{rename} operation before eventually synchronising.
The operations shown in \Cref{fig:GC-execution} are solely \emph{rename} operations, as only these operations are relevant to the problem at hand.
In \Cref{fig:GC-epoch-trees-step-1}, we represent the states of their respective \emph{epoch trees} once they each generate their \emph{rename} operations.
In \Cref{fig:GC-epoch-trees-step-2}, we represent the new states of their \emph{epoch trees} once they each receive the first \emph{rename} operation issued by each other.
...
Epochs that are no longer necessary by applying \Cref{rule:gc-concurrent-primary-epoch} and \Cref{rule:gc-root} are respectively displayed as blue dashed nodes and green dotted nodes.
...
\Cref{rule:gc-concurrent-primary-epoch} enables node A to garbage collect epochs \epoch{A8} then \epoch{A1}.
Then \Cref{rule:gc-root} enables node A to garbage collect \epoch{0} and the \emph{renaming} operation to switch to \epoch{B2}.
...
Nodes will then be able to garbage collect all other epochs using \Cref{rule:gc-concurrent-primary-epoch} and \Cref{rule:gc-root}, effectively suppressing the overhead of the renaming mechanism.
        ...
        \begin{minipage}{\linewidth}
            ...
            \begin{tikzpicture}[scale=0.8,every node/.style={scale=0.8}]
                ...
                \path
                    ...
                    to ++(0:1) node (b3) {}
                    to ++(0:1) node (b6) {}
                    to ++(0:1) node[point, label=above:{rename to \epoch{B7}}] (b7) {}
                    to ++(0:1.5)
                    to ++(270:1) node (hideBot) {}
                    to ++(90:1)
                    to ++(0:1.5) node[point, label=above:{rename to \epoch{B8}}] (b8) {}
                    to ++(0:1.5) node (b-end) {};
    ...
    \subfloat[States of respective epoch trees at (1)]{
        \begin{minipage}{\linewidth}
            ...
            \begin{tikzpicture}[scale=0.8,every node/.style={scale=0.8}]
                ...
                \draw[thick, ->] (AeA0) edge (AeA1) (AeA1) edge (AeA8);
                \draw[thick, ->] (BeB0) edge (BeB2) (BeB2) edge (BeB7);

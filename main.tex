...
To this end, $\trm{revertRenameId}$ maps identifiers from the \emph{child} epoch to equivalent ones in its \emph{parent} epoch.
...
To retrieve the original identifiers, $\trm{revertRenameId}$ simply uses their offset since it is their index the \emph{former state}.
...
As no corresponding identifier exists yet at the \emph{parent} epoch, $\trm{revertRenameId}$ can return any identifier as long as it preserves the intended order.

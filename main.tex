...
\acp{CRDT} are new specifications for existing data types, e.g. the Set or the Sequence.
While \acp{CRDT} display the same behaviour as previous specifications in sequential executions, they actually shine in distributed executions as they are designed to natively support concurrent modifications.
...
As a result, identifiers tend to grow as the system progresses.
This increase deepens the overhead of the \ac{CRDT} over time, leading to performance issues.
To address this issue in the context of real-time collaborative editing, we propose a new Sequence \ac{CRDT} which embeds a renaming mechanism.

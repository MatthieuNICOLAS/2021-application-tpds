...
We present our approach to make the \emph{rename} operation commutative with itself to handle scenarios with concurrent \emph{rename} operations.
...
\autoref{fig:conflicting-rename-operations} depicts such a scenario and expands the one previously described in \autoref{fig:concurrent-insert-rename-fixed}.
...
We represent in \autoref{fig:epoch-tree} the \emph{epoch tree} that nodes obtain once they eventually synchronise in the scenario described in \autoref{fig:conflicting-rename-operations}.
...
First, nodes have to be able to select one epoch from the \emph{epoch tree} as the target epoch.
...
This operation is delivered to node C, which later generates its own \emph{rename} operation creating the epoch \epoch{C6}.
...
Therefore each node selects \epoch{B7} as the target epoch in a coordination-free manner.

...
\mnnote{TODO: Reformuler phrase précédente}
% L'algo 1 permet aussi de résoudre le cas où le noeud B a inséré en concurrence du rename et doit maintenant intégrer le rename, lui permet de rename les identifiants insérés en concurrence
...
Therefore nodes can ignore several \emph{rename} operations from a set of concurrent \emph{rename} ones without introducing any anomalies.
...
We denote the one \emph{rename} operation that nodes apply from a set of concurrent \emph{rename} operations as the \emph{primary} one.
...
\mnnote{TODO: Reformuler/Simplifier 1er paragraphe}
% À partir du moment où on a des renames concurrents, les noeuds maintiennent un epoch tree et non plus une epoch chain
    ...
    \subfloat[State of both \emph{epoch trees} at (2)]{
        \begin{minipage}{.33\linewidth}
            ...
            \begin{tikzpicture}
                ...
                \path
                    to ++(270:1.5)
                    to ++(0:0.95) node[op, red] (eB2) {\epoch{B2}};

                \draw[thick] (e0) -- (eA1);
                \draw[thick] (e0) -- (eB2);
                \draw[->, >=stealth, dashed, thick, red] (eB2.180) -- (eA1.0) -- (e0.275);
            \end{tikzpicture}
...
As explained in \autoref{sec:dealing-with-concurrent-updates} and \autoref{sec:applying-primary-rename-op}, nodes have to store \emph{former states} corresponding to \emph{rename} operations to transform operations from previous or concurrent epochs.

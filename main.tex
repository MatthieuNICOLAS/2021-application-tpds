...
To prevent the generation of concurrent \emph{rename} operations, nodes have to rely on a consensus protocol.
However, consensus protocols suffer from their high cost and poor scalability.
To address these issues, previous work\cite{letia:hal-01248270,zawirski:hal-01248197} has proposed limiting the number of nodes participating in the consensus protocol by assigning this task to a small set of stable and well-connected nodes.
But this approach relies on organisations to provide these nodes to the system.
To make RenamableLogootSplit compatible with systems without central authorities, we present a mechanism to handle concurrent \emph{rename} operations in a coordination-free manner.
...
Our evaluation focuses on the comparison between RenamableLogootSplit and LogootSplit respective performance for following reasons.
First, it highlights the impact of the renaming mechanism on the performance of the replicated sequence.
Second, other Sequence \acp{CRDT} \cite{ROH2011354,briot:hal-01343941,lseq2017} suffer from drawbacks akin to LogootSplit's ones (tombstones, fragmentation into blocks of few elements, ever-growing identifiers).
As such, adapted versions of the proposed mechanism would offer them the same benefits than it provides to LogootSplit.
...
Third, a performance comparison with \cite{letia:hal-01248270,zawirski:hal-01248197} would be limiting since the main difference between both approaches lies in the systems for which they are designed.
The former is intended for decentralised systems and therefore relies on a consensus protocol, while RenamableLogootSplit avoids this constraint since it is designed for distributed systems without central authorities.
Thus, we instead discuss the comparison of the two approaches in more detail in \Cref{sec:core-nebula}.
...
However, systems can adopt the \emph{core-nebula} approach to simplify the implementation of RenamableLogootSplit.

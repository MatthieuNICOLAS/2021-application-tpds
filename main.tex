...
%
Finally, in order to compare the performance of RenamableLogootSplit to LogootSplit on a whole, we measured the time needed by both to replay a log of operations.
Obtained results are shown in \Cref{fig:time-to-replay-log}.
\mnnote{TODO: Figure est un placeholder, la remplacer une fois les résultats des benchmarks obtenus}

\begin{figure}[!ht]
    \centering
    \includegraphics[width=\columnwidth]{img/replay-log-30k.png}
    \caption{Evolution of time needed to replay operation log}
    \label{fig:time-to-replay-log}
\end{figure}

We observe that gains on integration times of \emph{insert} and \emph{remove} operations thanks to the renaming mechanism initially help to counterbalance the integration time \emph{rename} operations.
But as the collaboration progresses, \emph{rename} operations become more and more costly since more elements are involved.
This cost is multiplied in scenarios with concurrent \emph{rename} operations.
In such scenarios, RenamableLogootSplit becomes slower than LogootSplit to replay the log of operations.
It is thus necessary to improve the integration time of \emph{rename} operations.
...
We would thus frequently issue \emph{rename} operations, even if it means issuing concurrent \emph{rename} operations.

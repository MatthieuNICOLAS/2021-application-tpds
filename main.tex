...
Meanwhile, node B issues the \emph{rename} operation to \epoch{B2}.
...
\mnnote{NOTE: Un peu de mal à identifier si je dois parler au présent ou passé dans le paragraphe précédent vu que ça se passe en parallèle du paragraphe décrivant l'exécution du point de vue de A. Pareil pour si je dois utiliser "a" ou "the" pour parler des opérations \emph{rename} que j'ai déjà mentionné précédemment.}
...
The main outcome of these measures shows that \emph{rename} operations are expensive when compared to others as nodes have to browse and rename their whole current state.
...
Another interesting result from this benchmark is that \emph{concurrent with lower priority rename} operations are cheap to apply.
Since these operations introduce epochs that are not selected as the new current epoch, their application solely consists of storing their embedded \emph{former states}.

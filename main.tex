...
Identifiers hence tend to grow as the system progresses, which deepens the overhead of \acp{CRDT} over time and leads to performance issues.
...
The algorithm we present here enables nodes to rename their state identifier by identifier.
A possible extension is to design \textsc{renameBlock}, an improved version that renames the state block by block.
\textsc{renameBlock} would reduce the integration time of \emph{rename} operations, since its time complexity would no longer depends from the number of identifiers (\ie the number of elements) but from the number of blocks.
...
For example, identifiers of the form \id{pos}{nId~nSeq}{offset} (\id{i}{A1}{offset} in the current example) correspond to the new values of the identifiers which compose the \emph{former state}.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{olivier.jpg}}]{Olivier Perrin}
    is full professor at University of Lorraine, and works at LORIA in the COAST project. His research interests always been related to data and service management, and include various topics about collaborative systems, services composition, and distributed systems.

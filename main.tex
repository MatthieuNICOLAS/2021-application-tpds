...
In this paper, we focus on the variable-size identifiers approaches and more specifically on the LogootSplit algorithm.
...
For concurrently inserted identifiers, we explain in \Cref{sec:dealing-with-concurrent-updates} how nodes rename them in a deterministic way.
...
For instance, we built a peer-to-peer real-time collaborative editing system~\cite{MUTE2017} and we constrained the integration times of operations (including \emph{rename} ones) to be kept under the bound that corresponds to their perception by users.
